...
@WebServlet(urlPatterns = { "/servlets/UserProfile" })
	...
	protected void doPost(
		...
		try {
			...
			if (uub == null) {
				...
				request.getRequestDispatcher("../UserViews/user_profile_search.jsp").forward(request, response);
			} else {
				request.setAttribute("mail_address", mailAddress);
				request.setAttribute("password", password);
				request.setAttribute("uub", uub);
				request.getRequestDispatcher("../UserViews/user_profile.jsp").forward(request, response);
			}
		} catch (Exception e) {
